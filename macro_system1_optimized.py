...
Architecture
------------
Three independent sleeves, each sized by inverse volatility:

  Trend (40%)  — Cross-sectional or time-series momentum across 5 ETFs
                 (SPY, IEF, GLD, DBC, EFA).  Long-only or long/short.
  Carry (30%)  — DBV (G10 FX carry proxy).  Risk-on filter: SPY momentum
                 positive AND VIXY momentum negative; else SHY.
  Vol   (30%)  — SVXY (short-volatility / VRP proxy).  Same risk-on filter.

Overlays
--------
  • Sleeve risk balancing  : inverse-vol weighting normalises sleeve sizes.
  • Portfolio vol targeting: scales monthly returns to 10% ann. vol target
                             (leverage capped at 1.5×).
  • Gross leverage cap     : 1.25× maximum.
  • Drawdown guard         : blends to defensive (SHY) after −12% drawdown.

Data      : Yahoo Finance via yfinance (Adj Close, monthly resample)
Execution : Simulated monthly rebalancing, strict 1-month signal lag
Costs     : 10 bps × portfolio turnover each month

Walk-Forward Validation
-----------------------
To guard against in-sample overfitting, the optimiser is run on the
in-sample (IS) period only (START → WF_SPLIT_DATE).  The best IS config
is then applied unchanged to the out-of-sample (OOS) period
(WF_SPLIT_DATE → END) and results are reported side-by-side.

A large IS→OOS Sharpe drop (>50%) is flagged as a potential overfitting
warning.
"""
...
WF_SPLIT_DATE = "2020-01-01"  # Walk-forward: IS = START..WF_SPLIT_DATE-1, OOS = after
...
def rich_stats(rets_m: pd.Series, benchmark_rets: pd.Series | None = None) -> dict:
    """Full performance statistics via the shared performance module."""
    return compute_perf_stats(rets_m, freq=12, benchmark_returns=benchmark_rets)
...
def optimize_for_targets(
    ...
    tested = 0
    for cfg in build_candidate_configs():
        tested += 1
        try:
...
def walk_forward_validate(
    ...
    min_cagr: float = TARGET_MIN_CAGR,
    min_sharpe: float = TARGET_MIN_SHARPE,
    min_mdd: float = TARGET_MIN_MAX_DRAWDOWN,
) -> dict:
    """
    Walk-forward validation to measure out-of-sample robustness.

    Procedure
    ---------
    1. Optimise over the IN-SAMPLE (IS) period (START → split_date).
    2. Take the best IS config; run it unchanged on the full period
       and slice out the OUT-OF-SAMPLE (OOS) portion (split_date → END).
    3. Report IS and OOS stats side-by-side.
    4. Flag if OOS Sharpe drops >50% relative to IS (overfitting warning).

    Parameters
    ----------
    px_d       : Full price history downloaded once.
    split_date : IS/OOS boundary (ISO-format string).
    """
    split_ts = pd.Timestamp(split_date)

    # ---- Step 1: optimise on IS data only ----
    px_is = px_d.loc[px_d.index < split_ts]
    if len(px_is) < 252:
        raise RuntimeError(
            f"In-sample period has fewer than 252 daily rows "
            f"(split_date={split_date}, start={px_d.index[0].date()}). "
            "Widen the IS window."
        )

    print(f"\nWalk-forward: optimising on IS period "
          f"({px_d.index[0].date()} → {split_ts.date()})…")
    ...
    for cfg in build_candidate_configs():
        tested += 1
        try:
            res = run_single_backtest(px_d=px_is, config=cfg)
        except Exception:
            continue

        s = res["stats"]
        score = objective_score(s, min_cagr=min_cagr, min_sharpe=min_sharpe, min_mdd=min_mdd)
        if best_is is None or score > best_is_key:
            best_is = res
            best_is_key = score

    if best_is is None:
        raise RuntimeError("No valid IS backtest completed.")

    print(f"  Tested {tested} configs on IS data. Best IS Sharpe: {best_is['stats']['Sharpe']:.2f}")

    # ---- Step 2: apply best IS config to full history, slice OOS ----
    best_cfg = best_is["config"]
    full_res = run_single_backtest(px_d=px_d, config=best_cfg)

    full_rets = full_res["portfolio_returns_final"]
    oos_rets = full_rets.loc[full_rets.index >= split_ts]

    if len(oos_rets) < 12:
        raise RuntimeError(
            f"OOS period has fewer than 12 monthly observations "
            f"(split_date={split_date}). Choose an earlier split."
        )

    is_rets = full_rets.loc[full_rets.index < split_ts]

    # ---- Step 3: benchmark (SPY monthly returns) ----
    spy_m: pd.Series | None = None
    if "SPY" in px_d.columns:
        spy_m = px_d["SPY"].resample("ME").last().pct_change().dropna()

    def slice_bm(bm, idx):
        if bm is None:
            return None
        return bm.reindex(idx).dropna()

    is_stats = compute_perf_stats(is_rets, freq=12, benchmark_returns=slice_bm(spy_m, is_rets.index))
    oos_stats = compute_perf_stats(oos_rets, freq=12, benchmark_returns=slice_bm(spy_m, oos_rets.index))
    full_stats = compute_perf_stats(full_rets, freq=12, benchmark_returns=slice_bm(spy_m, full_rets.index))

    # ---- Step 4: overfitting diagnostic ----
    is_sharpe = is_stats["Sharpe Ratio"]
    oos_sharpe = oos_stats["Sharpe Ratio"]
    sharpe_decay = (is_sharpe - oos_sharpe) / (abs(is_sharpe) + 1e-12)
...
if __name__ == "__main__":
    ...
    requested = sorted(set(TREND_TICKERS + [CARRY_TICKER, VOL_TICKER, HEDGE_TICKER, DEFENSIVE]))
    print(f"Downloading prices for: {requested}")
    px_d = download_adj_close(requested, start=START, end=END)

    # ---- 2. Walk-forward validation ----
    wf = walk_forward_validate(
        px_d=px_d,
        split_date=WF_SPLIT_DATE,
        min_cagr=TARGET_MIN_CAGR,
        min_sharpe=TARGET_MIN_SHARPE,
        min_mdd=TARGET_MIN_MAX_DRAWDOWN,
    )

    # ---- 3. Print overfitting diagnostic ----
    print(f"\nWalk-Forward Split : {wf['split_date']}")
    print(f"IS Sharpe          : {wf['is_sharpe']:.2f}")
    print(f"OOS Sharpe         : {wf['oos_sharpe']:.2f}")
    decay_pct = wf["sharpe_decay"] * 100
    if wf["sharpe_decay"] > 0.5:
        print(f"⚠  Sharpe decay {decay_pct:.0f}% — possible overfitting to IS period.")
    elif wf["sharpe_decay"] < 0:
        print(f"✓  OOS Sharpe ({wf['oos_sharpe']:.2f}) exceeds IS — robust out-of-sample.")
    else:
        print(f"✓  Sharpe decay {decay_pct:.0f}% — within acceptable range.")

    # ---- 4. Full performance comparison table ----
    full_res = wf["full_result"]
    full_rets = full_res["portfolio_returns_final"]
    spy_m = wf["spy_m"]

    spy_stats: dict | None = None
    if spy_m is not None:
        spy_stats = compute_perf_stats(
            spy_m.reindex(full_rets.index).dropna(), freq=12
        )

    all_stats: dict[str, dict] = {
        f"Macro System (IS {START}–{WF_SPLIT_DATE[:4]})": wf["is_stats"],
        f"Macro System (OOS {WF_SPLIT_DATE[:4]}–present)": wf["oos_stats"],
        f"Macro System (Full)": wf["full_stats"],
    }
    if spy_stats is not None:
        all_stats["SPY (B&H, Full)"] = spy_stats

    print_stats_table(
        all_stats,
        title="Macro System: Trend + Carry + Vol — Walk-Forward Performance",
    )

    # ---- 5. Selected config ----
    print("\n=== Optimised Configuration ===")
    for k, v in wf["best_config"].items():
        print(f"  {k:<35}: {v}")

    print("\n=== Diagnostics ===")
    for k, v in full_res["diagnostics"].items():
        print(f"  {k:<35}: {v}")

    if wf["sharpe_decay"] > 0.5:
        print(
            "\nWARNING: The IS→OOS Sharpe decay exceeds 50%. "
            "Consider expanding the OOS window or reducing the parameter search space "
            "to improve out-of-sample confidence."
        )

    # ---- 6. Tearsheet ----
    plot_tearsheet(
        returns=full_rets,
        benchmark_returns=spy_m,
        title="Macro System: Trend + Carry + Vol (Walk-Forward Optimised)",
        freq=12,
        rolling_window=12,
        save_path="macro_system_tearsheet.png",
        show=True,
    )
